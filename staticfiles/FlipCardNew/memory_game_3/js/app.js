...
try {
	...
	if (err) {
        ...
        for(let i = 1; i <= 16; i++) {
            ...
            if (i > 8) {
                let j = i - 8;
                svg = '<svg role="img" class="icon" title="0' + j + '"><use xlink:href="/static/FlipCardNew/memory_game_3/img/sprites.svg#icon-' + j + '"></use></svg>';
                li.innerHTML = svg;
            } else {
                svg = '<svg role="img" class="icon" title="0' + i + '"><use xlink:href="/static/FlipCardNew/memory_game_3/img/sprites.svg#icon-' + i + '"></use></svg>';
                li.innerHTML = svg;
            }
            cardsList.push(li);
        }
